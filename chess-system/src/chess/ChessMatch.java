...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (movedPiece instanceof Pawn) {
			if ((movedPiece.getColor() == Color.WHITE && target.getRow() == 0) || (movedPiece.getColor() == Color.BLACK && target.getRow() == 7)) {
				promoted = (ChessPiece)board.piece(target); // indicando o peão a peça promovida;
				promoted = replacePromotedPiece("Q"); // por padrão, vai promover p rainha; dps pergunta p usuário se quer trocar; pq por padrão, o promoted é nulo, então foi dado o peão para ser promovido;
			}
		}
	...
	private ChessPiece newPiece(String type, Color color) { // método auxiliar para instanciar a peça que foi colocada no promoted;
		if (type.equals("B")) return new Bishop(board, color);
		if (type.equals("N")) return new Knight(board, color);
		if (type.equals("Q")) return new Queen(board, color);
		return new Rook(board, color); // se falhou tds acima, cm crtz vai ser uma torre;
	}
